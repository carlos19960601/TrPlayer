...
class Whisper {
  ...
  constructor() {
    this.binFile = path.join(__dirname, "lib", "whisper", os.platform() === "win32" ? "main.exe" : "main")
  }
      ...
      proc.on("close", (code) => {
        ...
        if (fs.existsSync(`${outputFile}.json`)) {
          ...
          if (stat.size === 0) {
            ...
            const whisperResult = fs.readJSONSync(`${outputFile}.json`) as WhisperResult;
  ...
  registerIpcHandlers() {
    ipcMain.handle("whisper-recognize", (event: IpcMainInvokeEvent, params: TranscribeParamsType) => {
